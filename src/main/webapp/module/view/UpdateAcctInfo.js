Ext.define('Fes.view.UpdateAcctInfo', {
	extend : 'Ext.grid.Panel',
	alias : 'widget.updateacctinfo',
	title : '菜单列表',
...
/*				field : {
	...
	initComponent : function() {
		...
		this.rowEditor = Ext.create('Ext.grid.plugin.CellEditing', {
			id : 'acctinfoRowEditor',
			clicksToEdit : 2,
			listeners : {
				edit : function(editor, e) {
					Ext.apply(me.store.proxy.extraParams, {
								'rschacctno' : Ext.getCmp('acctrschacctno')
										.getValue()
							});
			...
			listeners : {
				"beforechange" : function(bbar) {
					...
					Ext.apply(store.proxy.extraParams, {
								'rschacctno' : Ext.getCmp('urschacctno')
										.getValue()
							});
				}
			}
	...
	createStore : function() {
		var me = this;
		this.store = Ext.create('Fes.store.UpdateAcctInfoStore');
	},
	searchRecord : function() {
		...
		if (Ext.isEmpty(Ext.util.Format.trim(Ext.getCmp('acctrschacctno')
				.getValue()))) {
			Ext.Msg.show({
						title : '修改贷款信息',
						msg : '账号不能为空！',
			...
			me.getStore().load({
				params : {
					rschacctno : Ext.getCmp('acctrschacctno').getValue(),
					page : 1,
					start : 0
				},
