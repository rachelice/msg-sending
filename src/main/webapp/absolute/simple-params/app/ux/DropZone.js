/**
 * @class SimpleParams.ux.DropZone
 * @extends Ext.tree.ViewDropZone
 * @private
 */
Ext.define('SimpleParams.ux.DropZone', {
    extend: 'Ext.tree.ViewDropZone',

    handleNodeDrop : function(data, overRecord, position) {
        var droppedRecord = data.records[0];

        if(droppedRecord instanceof SimpleParams.model.Param) {
            this.cancelExpand();
            this.fireViewEvent('paramdrop', droppedRecord, overRecord);
        } else if(droppedRecord instanceof SimpleParams.model.List) {
    ...
    onNodeOver: function(node, dragZone, e, data) {
        ...
        // Otherwise if we're dragging a Param from the Param Grid, do not show the indicator, since we want
        // to give the appearance of the dragged record being dropped ON a node, not in between nodes.
        if(data.records[0] instanceof SimpleParams.model.List) {
