/**
 * @class SimpleReports.controller.Lists
 * @extends Ext.app.Controller
 */
Ext.define('SimpleReports.controller.Lists', {
    ...
    addList: function(leaf) {
        var me = this,
            ...
            newList = Ext.create('SimpleReports.model.List', {
            	parName: '新 ' + (leaf ? '报表' : '目录'),
                leaf: leaf,
                ...
                if(response === 'yes') {
                    ...
                    (function deleteTasks(list) {
                        tasks = tasksStore.queryBy(function(task, id) {
                            return task.get('parentParamId') === list.get('id');
                        });
     ...
     * @param {SimpleReports.model.List[]} lists
     */
    filterTaskGrid: function(selModel, lists) {
    	var me=this;
    	console.count('filterTaskGrid.........lists.length'+lists.length);
        ...
        // build an array of all the parentParamId's in the hierarchy of the selected list
        list.cascadeBy(function(list) {
            listIds.push(list.get('id'));
        });

        // remove any existing "parentParamId" filter from the filters array
        for(; i < filterCount; i++) {
            if(filters[i].property === 'parentParamId') {
                filters.splice(i, 1);
                filterCount --;
            }
        }
        // add the new parentParamIds to the filters array
        filters.push({ property: "parentParamId", value: new RegExp('^' + listIds.join('$|^') + '$') });
        ...
        this.getTaskGrid().setTitle(list.getPath('parName','》')+list.get('parName'));
       ...
       if(this.getTaskForm().query('[name=parentParamId]')[0]){
    	   
    	   this.getTaskForm().query('[name=parentParamId]')[0].setValue(list.get('id'));
       }
     ...
     * @param {SimpleReports.model.Task} task       The Task record that was dropped
     * @param {SimpleReports.model.List} list       The List record that the mouse was over when the drop happened
     */
    updateTaskList: function(task, list) {
        var me = this,
            listId = list.get('id');

        // set the tasks parentParamId field to the id of the list it was dropped on
        task.set('parentParamId', listId);
     ...
     * @param {SimpleReports.store.Tasks} tasksStore
     ...
     * @param {SimpleReports.store.Lists} listsStore
     * @param {SimpleReports.model.List[]} lists
     * @param {Boolean} success
     * @param {Ext.data.Operation} operation
     */
     ...
     * @param {SimpleReports.view.lists.Tree} listTree
     */
