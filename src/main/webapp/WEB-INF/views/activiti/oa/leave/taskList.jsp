...
<head>
	...
	<script src="${ctx }/activiti/js/module/oa/leave/leave-todo.js" type="text/javascript"></script>
</head>
		...
		<tbody>
			<c:forEach items="${page.result }" var="leave">
				...
				<tr id="${leave.id }" tid="${task.id }">
					...
					<%--<td><a target="_blank" href='${ctx }/activiti/workflow/resource/process-instance?pid=${pi.id }&type=xml'>${task.name }</a></td> --%>
					<td>${task.createTime }</td>
					<td>${pi.suspended ? "已挂起" : "正常" }；<b title='流程版本号'>V: ${leave.processDefinition.version }</b></td>
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/activiti/oa/leave/task/claim/${task.id}">签收</a>
						</c:if>
