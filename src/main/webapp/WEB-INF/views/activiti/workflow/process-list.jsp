...
<body>
	...
	<div style="text-align: right;padding: 2px 1em 2px">
		...
		<a id='redeploy' href='${ctx }/activiti/workflow/redeploy/all' style="display:none">重新部署流程</a>
	</div>
	<fieldset id="deployFieldset" style="display: none">
		<legend>部署新流程</legend>
		<div><b>支持文件格式：</b>zip、bar、bpmn、bpmn20.xml</div>
		<form action="${ctx }/activiti/workflow/deploy" method="post" enctype="multipart/form-data">
		...
		<tbody>
			<c:forEach items="${page.result }" var="object">
				...
				<tr>
					...
					<td>
                        <a href='${ctx }/activiti/workflow/process/delete?deploymentId=${process.deploymentId}'>删除</a>
                        <a href='${ctx }/activiti/workflow/process/convert-to-model/${process.id}'>转换为Model</a>
                    </td>
				</tr>
			</c:forEach>
		</tbody>
